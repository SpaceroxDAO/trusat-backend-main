...
# app.config['MYSQL_DB'] = 'space'
# mysql = MySQL(app)
# import mysql.connector
# conn = mysql.connector.connect(
#    user='root', password='!123456!', host='127.0.0.1', database='space',auth_plugin='mysql_native_password'
# )
...
def before_request_func():
    ...
    CONFIG = os.path.abspath("../trusat-backend/trusat-config.yaml")
...
def catalog_weather(catalog, offset):
    ...
    all_catalogs = {
        ...
        "featured": g.get('db').selectCatalog_Category_JSON,
        "visual": g.get('db').selectCatalog_Category_JSON,
        "starlink": g.get('db').selectCatalog_Category_JSON,
        "one-web": g.get('db').selectCatalog_Category_JSON,
        "active": g.get('db').selectCatalog_Category_JSON,
        "analyst": g.get('db').selectCatalog_Category_JSON,
        "weather-earth": g.get('db').selectCatalog_Category_JSON,
        "weather": g.get('db').selectCatalog_Category_JSON,
        "noaa": g.get('db').selectCatalog_Category_JSON,
        "goes": g.get('db').selectCatalog_Category_JSON,
        "resource": g.get('db').selectCatalog_Category_JSON,
        "sarsat": g.get('db').selectCatalog_Category_JSON,
        "disaster-monitoring": g.get('db').selectCatalog_Category_JSON,
        "tracking-and-data-relay": g.get('db').selectCatalog_Category_JSON,
        "argos": g.get('db').selectCatalog_Category_JSON,
        "planet": g.get('db').selectCatalog_Category_JSON,
        "spire": g.get('db').selectCatalog_Category_JSON,
        "communications": g.get('db').selectCatalog_Category_JSON,
        "geo": g.get('db').selectCatalog_Category_JSON,
        "geo-protected-zone": g.get('db').selectCatalog_Category_JSON,
        "geo-protected-zone-plus": g.get('db').selectCatalog_Category_JSON,
        "intelsat": g.get('db').selectCatalog_Category_JSON,
        "ses": g.get('db').selectCatalog_Category_JSON,
        "iridium": g.get('db').selectCatalog_Category_JSON,
        "iridium-next": g.get('db').selectCatalog_Category_JSON,
        "orbcomm": g.get('db').selectCatalog_Category_JSON,
        "globalstar": g.get('db').selectCatalog_Category_JSON,
        "amateur-radio": g.get('db').selectCatalog_Category_JSON,
        "experimental": g.get('db').selectCatalog_Category_JSON,
        "other-comm": g.get('db').selectCatalog_Category_JSON,
        "satnogs": g.get('db').selectCatalog_Category_JSON,
        "gorizant": g.get('db').selectCatalog_Category_JSON,
        "raduga": g.get('db').selectCatalog_Category_JSON,
        "molniya": g.get('db').selectCatalog_Category_JSON,
        "navigation": g.get('db').selectCatalog_Category_JSON,
        "gps-ops": g.get('db').selectCatalog_Category_JSON,
        "glonass-operational": g.get('db').selectCatalog_Category_JSON,
        "galileo": g.get('db').selectCatalog_Category_JSON,
        "beidou": g.get('db').selectCatalog_Category_JSON,
        "satellite-based-augmentation": g.get('db').selectCatalog_Category_JSON,
        "system-navigation": g.get('db').selectCatalog_Category_JSON,
        "musson": g.get('db').selectCatalog_Category_JSON,
        "debris": g.get('db').selectCatalog_Category_JSON,
        "indian-asat-test": g.get('db').selectCatalog_Category_JSON,
        "iridium-33": g.get('db').selectCatalog_Category_JSON,
        "cosmos-2251": g.get('db').selectCatalog_Category_JSON,
        "science": g.get('db').selectCatalog_Category_JSON,
        "geodetic": g.get('db').selectCatalog_Category_JSON,
        "engineering": g.get('db').selectCatalog_Category_JSON,
        "education": g.get('db').selectCatalog_Category_JSON,
        "misc": g.get('db').selectCatalog_Category_JSON,
        "military": g.get('db').selectCatalog_Category_JSON,
        "radar-calibration": g.get('db').selectCatalog_Category_JSON,
        "cubesat": g.get('db').selectCatalog_Category_JSON,
        "stations": g.get('db').selectCatalog_Category_JSON,
        "other": g.get('db').selectCatalog_Category_JSON,
        "all": g.get('db').selectCatalog_All_JSON
    }
    try:
        if catalog == "all" or catalog == "priorities" or catalog == "undisclosed" or catalog == "latest":
...
def get_nonce():
    ...
    try:
        ...
        # cursor = conn.cursor()
        # results = cursor.execute("SELECT Observer.eth_addr FROM Observer INNER JOIN Observer_email ON Observer.id=Observer_email.user_id WHERE Observer_email.email='"+email+"'LIMIT 1")
        if len(results) == 42:
            return {}
    except Exception as e:
        print(e)
